...
- **Excel and Parquet open inside an archive and from a URL too.** Both are read through a reader that takes bytes, so neither has to be a file. The ceilings are unchanged: a workbook out of an archive is still read up to 64MB, and one at a URL passes the 512MB download ceiling before that.
...
- SQLite entries inside an archive are not opened. A database is read by **querying** a file — held open, its pages read as a statement asks for them, its journal written beside it. Something unpacked from an archive has no file to query. Extract it first and open that file.
