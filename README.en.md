...
- File watching and reloading are best effort and retain mmap. Updates follow saves without a timing guarantee; deletion and rename-out are ignored. Reading an old mapping during an external save can mix values or terminate the process (Unix SIGBUS or Windows in-page error). In-place truncation and rename replacement can succeed on Windows too. Reloading does not eliminate the risk of mapping changes.
