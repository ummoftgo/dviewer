...
- Markdown HTML copy uses the sanitised representation before post-processing; it excludes rendered Mermaid/KaTeX output and app styles. Source copy is disabled for blocks without a verifiable source position, including raw HTML. Copying a single block does not append reference definitions automatically. Language choices last for the open tab and reset on reinterpretation. The bundled grammars do not include TypeScript or TOML; re-highlighting also has a 16MiB input limit. Source slicing preserves line endings, but the Windows plain-text clipboard may convert LF to CRLF.
