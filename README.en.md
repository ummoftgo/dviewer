...
A desktop viewer for HTML, PDF, Markdown, JSON, JSONC, JSONL, YAML, TOML, XML, CSV, TSV, plain text or logs, SQLite, Excel, Parquet, and ZIP. Tauri v2 + Rust backend + Svelte 5 frontend. The interface is available in Korean, English, Japanese and Simplified Chinese.

Sixteen formats, grouped into **six** ways of reading.
...
| Frame | HTML (.html/.htm/.xhtml) · PDF | Scripts, local modules and web fonts in an isolated origin; TOC, Ctrl+F search, Ctrl+E line-indexed source, relative document links, per-document external-resource controls, and sibling resources for HTML in local ZIP files. PDF includes page/zoom/rotation/search controls, selection/copy, outlines and text/image orientation correction with reverse and undo |
...
Session restoration is enabled by default. The main window reopens local files and URLs in tab order, preserving the active document, source/rendered mode, and reading position, then opens startup requests. Markdown resumes within a heading section; grids use original row numbers, collections also retain the sheet/table name, trees use a saved selection path, source views use line numbers, HTML restores its scroll ratio after loading, and PDF restores its page number and rotation. A brief status message confirms a restored position below the top. Archive entries collapse to their local root file. Pasted text, derived tabs, other windows, archive list positions, sorting, and filters are not restored; `--new` and normal smoke startup skip session restoration and saving.

Positions are saved after scrolling stops for one second, and immediately on tab switches or closes. Movement in the final second before exit may be lost. If changed content no longer contains the saved position, reading starts at the top. Tree coordinates compare complete source keys, child ordinals, and node kinds; paths exceeding 64 KiB or 1,024 segments are not saved. Changes to child order or key spelling can prevent restoration.
...
Markdown tables in Fill mode wrap long URLs and inline code within their cells. Recommendations are redistributed to fit the document width; horizontal overflow is allowed only when the combined minimum column widths exceed it. Code blocks inside cells keep their own horizontal scrolling, and the table's content-width mode is unchanged.
...
- PDF support is Windows-only in this release, with a 256 MiB limit. PDF.js provides page navigation, zoom, rotation, search, text selection/copy and outlines; sessions restore the page number and rotation. The default angle of a document that opened uncorrected is not saved. Without a saved rotation, the first three pages determine automatic orientation correction: at least 20 nonblank horizontal-writing text items and at least 80% sharing one direction. Correction applies to the whole document and can be undone in the status bar. When there are too few text items, up to two pages are rendered with a longest edge of 512px, and pixels below luminance 200 count as ink. Image correction applies only when all sampled pages agree, ink covers at least 1%, and column-profile variance is at least twice row-profile variance. The turn, 90° or 270°, follows the steadier edge of the lines (where left-aligned lines begin); without a clear edge, or when the two pages disagree, it is 270°. The first page appears at once, but the outline and the restored position wait for the image probe, so it has a 500ms budget; slow, failed or ambiguous probes are skipped. Documents with large images or thousands of outline paths may miss the correction on a slow PC. Image inference can be wrong for photos or diagrams, and justified or centred documents may come out upside down; use Reverse or Undo in its status pill. Vertical-writing fonts are excluded from text detection, and mixed-orientation documents are not corrected page by page. Source view, editing, OCR and password-protected documents are unsupported. Pages without text show a search/copy notice. Searching may read every PDF page and can be expensive on large documents. Opening PDFs on macOS or Linux returns an unsupported-format error. The static file associations still include pdf. WebKit initialization investigation continues for a later release.
...
- Local HTML sibling resources stay inside the file’s parent directory; symbolic links and path escapes are rejected. HTML in a local ZIP can read siblings within the archive root while its direct parent ZIP is open in the same window, with a 64MiB limit enforced during decompression of each resource. Closing the parent leaves the HTML body available but prevents subsequent sibling requests. Siblings in nested ZIPs, ZIPs downloaded from URLs, and URL documents are unsupported.
