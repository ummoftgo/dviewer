...
const pending = {frameServed:null, frameCsp:[], frameAgentStarted:false, frameStage:null, frameStall:null, frameOrientation:null};
...
test('request counts, parent CSP and agent start distinguish the second timeout stage without URLs', () => {
  ...
  const diagnostic = frameDiagnostic({frameUrlPort:'46199', frameLoaded:true, frameReady:false, frameError:null,
    frameServed:{html:1, agent:1, resource:2,last:[]}, frameCsp:[csp], frameAgentStarted:true, frameStage:null,frameStall:null,frameOrientation:null});
...
test('PDF timeout retains the last stage and error detail without document credentials', () => {
  ...
  expect(frameDiagnostic({...pending,frameUrlPort:'43123',frameLoaded:true,frameReady:false,frameStage:'worker-start',
    ...
    .toBe('(frame: url ok, port 43123, load yes, ready no, error PDF failed (detail Failed to import [url] [token]), served unknown, csp -, agent start no, stage worker-start, last: -, stall -, orientation -)');
});
