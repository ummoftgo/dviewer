...
```bash
cd src-tauri && cargo test          # 404개: 스캐너 둘, 가시성, 검색, 변환, 표, 인코딩, 판별, 아카이브, 표시/복사, 정화
...
`npm test` 는 vitest 를 돌립니다. 테스트는 검사하는 코드 **옆에** 삽니다(`src/**/*.test.ts`) — Rust 쪽 404개가 전부 같은 파일 안 `#[cfg(test)] mod tests` 에 있는 것과 같은 문법입니다.
...
읽히는 것 셋입니다. **뒤쪽에 닿는 것은 걷기**입니다 — 자식이 색인에서 이어 붙어 있어 n번째에 가려면 그 앞을 밟고, 그 행까지 스크롤하는 것과 같은 값입니다. **슬라이스는 그 걷기를 한 번만** 합니다(`[1000000:1000010]` 이 `[1000000]` 과 같은 값입니다). 그리고 **필터는 답이 아니라 파일을 따라갑니다** — 같은 아홉 건을 `$.items` 아래에서 0.35초, `$..` 아래에서 2.5초에 찾습니다. 그것이 이 한 단계에만 취소가 붙어 있는 이유이고, 4,096 노드마다 깃발을 보는 값이 0.3ms 입니다.
