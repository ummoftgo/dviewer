...
| 항목 | 값 |
|---|---|
| 열기 (시트 목록) | 0.28ms — 2.9KB 파일에서도 0.28ms. 파일에 처음 닿는 실행은 87ms |
| 시트 선택 (값 전부) | 0.36초 |
| 메모리에 올라간 값 | **51MB** |
...
**여는 값은 시트 목록뿐이고, 크기를 따라가지 않습니다.** 9.7MB 와 2.9KB 가 같은 0.28ms 입니다 — calamine 이 여는 순간 읽는 것은 `workbook.xml` 과 관계 파일이고, 시트의 XML 은 고를 때 풀립니다.

같은 표의 이 자리에 한동안 **93ms** 가 적혀 있었는데, 그것은 파일에 처음 닿는 실행의 값입니다. 이번에 고친 코드로 재면서도 첫 실행은 87ms 였고 이어지는 실행은 전부 0.28ms 였습니다 — 같은 바이너리, 같은 파일. 재고 있던 것은 여는 비용이 아니라 9.7MB 를 처음 디스크에서 끌어오는 비용이었습니다.
