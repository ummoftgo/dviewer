...
```bash
cd src-tauri && cargo test          # 385개: 스캐너 둘, 가시성, 검색, 변환, 표, 인코딩, 판별, 아카이브, 표시/복사, 정화
...
`npm test` 는 vitest 를 돌립니다. 테스트는 검사하는 코드 **옆에** 삽니다(`src/**/*.test.ts`) — Rust 쪽 385개가 전부 같은 파일 안 `#[cfg(test)] mod tests` 에 있는 것과 같은 문법입니다.
...
매니페스트의 33개를 **정상 열기 파이프라인 그대로** — `workspace.openPath` 를 부르고, 진짜 뷰가 마운트되어 스스로 로드하기를 기다립니다. 그 위에 파일 하나로는 닿지 않는 둘:
...
그리고 **단일 인스턴스 왕복** — 두 번째 프로세스를 실제로 띄워 ① 스스로 종료하는가 ② 첫 프로세스에 인자가 도착했는가, 그리고 `--new` 로 한 번 더 ③ 새 창이 정말 부팅했는가. 세 번째가 `--new` 빈 창 결함의 직접 재현입니다.
